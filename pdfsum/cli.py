...
def _summarize_one(pdf_path: str, num_sentences: int, language: str) -> str:
    ...
    result = summarize_detailed(text, num_sentences=num_sentences, language=language)

    lines = [f"# {title}", ""]
    if result.topic_line:
        lines.extend([result.topic_line, ""])
    if result.overview:
        lines.extend([result.overview, ""])
    if result.sentences:
        lines.extend(f"- {s}" for s in result.sentences)
    else:
        lines.append("(no se pudo generar un resumen)")
    return "\n".join(lines)
